    ...
    def run(self):
        ...
        for _ in tqdm(range(self.n_batch), desc=f"{self.n_batch} batches"):
            ...
            for bin in self.cmac_bins:
                for cov_mat, numeric_idxs in zip(self.cov_mat_dict[bin], self.gene_numeric_idxs[bin]):
                    start_time = time.perf_counter()
                    burden_stats = variant_set_test_njit(
                        half_score, 
                        cov_mat,
                        numeric_idxs,
                    )
                    ...
                    if not to_compile:
                        ...
                        to_compile = False
                    burden_stats[burden_stats <= self.sig_thresh] = 0
                    burden_stats = csr_matrix(burden_stats)
...
def variant_set_test_njit(
    half_score, cov_mat_dict_bin, gene_numeric_idxs_bin
):
    n_genes = len(gene_numeric_idxs_bin)
    burden_stats = np.zeros((n_genes, half_score.shape[1]), dtype=np.float32)
    
    for idx in numba.prange(n_genes):
        for i in gene_numeric_idxs_bin[idx]:
            burden_stats[idx] += half_score[i]
        burden_stats[idx] = burden_stats[idx] ** 2 / cov_mat_dict_bin[idx]
...
class CreatingMask:
    ...
    def __init__(
        ...
        self.n_masks = {k:sum(len(x) for x in v) for k, v in self.gene_numeric_idxs.items()}
    ...
    def _parse_genes_sliding_window(self):
        """
        Get independent genes for each cMAC bin, TODO: faster
        ...
        for bin in self.cmac_bins:
            ...
            while True:
                ...
                while start + window_size < self.n_variants:
                    ...
                    start += skip_size
                if output:
                    gene_numeric_idxs[bin].append(np.array(output))
                if bin_count >= 10000:
                    break
    ...
    def _sumstats(self):
        """
        Calculating summary statistics for burden test, TODO: faster
        ...
        for bin, numeric_idx_list in self.gene_numeric_idxs.items():
            for numeric_idx in numeric_idx_list:
                cov_mat = np.zeros(len(numeric_idx), dtype=np.float32)
                for i, idx in enumerate(numeric_idx):
                    x = vset_half_covar_proj[idx]
                    vset_ld = self.vset_ld[idx][:, idx]
                    cov_mat[i] = np.array(vset_ld - np.dot(x, x.T)).sum()
                cov_mat_dict[bin].append(cov_mat)
...
def run(args, log):
    ...
    if args.perm_list is not None:
        ...
        try:
            ...
            #                 sig_stats = sig_stats[-int(args.sig_thresh * burden_count_dict[bin]):]
            #             bin_str = "_".join(str(x) for x in bin) + "_" + str(voxel)
            #             dataset = file.create_dataset(bin_str, data=sig_stats)
            #             dataset.attrs["count"] =  burden_count_dict[bin]
